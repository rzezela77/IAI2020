...
# Dados da Seccao H 
ibsa_sec_h_dat <- read_dta("shiny_app/00_data/sec_hv1.dta")

ibsa_sec_h_dat %>% glimpse()


# to_factor(ibsa_sec_h_dat$d01) %>% unique()


# adicionar nova variavel: prov_dist
ibsa_sec_h_dat$prov_dist <- ibsa_sec_h_dat$prov*100 + ibsa_sec_h_dat$dist
...
is.element(producao_sec_h_provincia_tbl$PROVINCIA, moz_province$PROVINCIA) %>% 
    all()
...
# Tipo de Cultura
producao_sec_h_provincia_tbl$tipo_cultura %>% unique()

# Medir o nivel de producao por tipo de cultura
producao_sec_h_provincia_tbl %>% 
    group_by(tipo_cultura) %>% 
    summarise(total_geral = sum(total_qnt_kg )) %>% 
    arrange(desc(total_geral)) %>% 
    ungroup()

# criar tabela por tipo de cultura ----------------------------------------
tipo_producao_sec_h_provincia_tbl <-
    producao_sec_h_provincia_tbl[producao_sec_h_provincia_tbl$tipo_cultura == "FEIJAO NHEMBA",]
...
# tipo_producao_sec_h_provincia_merged_tbl %>% glimpse()
...
# Visualizar a Producao por Distrito  -------------------------------------

# criar tabela por tipo de cultura
tipo_producao_sec_h_distrito_tbl <-
    producao_sec_h_distrito_tbl[producao_sec_h_distrito_tbl$tipo_cultura == "FEIJAO NHEMBA", ]
...
tipo_producao_sec_h_distrito_merged_tbl <- merge(moz_district, tipo_producao_sec_h_distrito_tbl, by = "prov_dist", all.x = F)

# tipo_producao_sec_h_distrito_merged_tbl@data %>% glimpse()

# Mapa por distritos usando Palette ----
paletteNum <- colorNumeric('Greens', domain = tipo_producao_sec_h_distrito_merged_tbl$total_qnt_kg/1000)


producao_Labels <- sprintf('<b>%s</b><br/>%s ton.<br/>%s',
                           tipo_producao_sec_h_distrito_merged_tbl$DISTRITO, 
                           prettyNum(round(tipo_producao_sec_h_distrito_merged_tbl$total_qnt_kg/1000, 2),big.mark = ","),
                           tipo_producao_sec_h_distrito_merged_tbl$tipo_cultura) %>%
...
m_distritos <- leaflet() %>%
    ...
    setView(lng = 35.529562, lat = -18.665695, zoom = 4) %>% 
    addPolygons(data = tipo_producao_sec_h_distrito_tbl,
                ...
                # specify that the each state should be colored per paletteNum()
                fillColor = ~paletteNum(tipo_producao_sec_h_distrito_merged_tbl$total_qnt_kg/1000),
    ...
    addLegend(pal = paletteNum, values = tipo_producao_sec_h_distrito_merged_tbl$total_qnt_kg/1000, title = paste0("'<small>Campanha Agricola 2020 <br> Producao de ", unique(tipo_producao_sec_h_distrito_merged_tbl$tipo_cultura), " (ton.)<br>(fonte: IAI 2020)</small>'"), position = 'bottomright')
...
# Tabelas: Producao por Provincia
# producao_sec_h_provincia_dta <- 
# producao_sec_h_provincia_tbl %>% 
#     filter(tipo_cultura == "AMENDOIM PEQUENO") %>% 
#     group_by(provincia=PROVINCIA, tipo_cultura) %>% 
#     summarise(total_qnt_tons = total_qnt_kg/1000) 

producao_sec_h_provincia_dta <- 
    tipo_producao_sec_h_provincia_merged_tbl@data %>% 
    group_by(provincia = PROVINCIA, tipo_cultura) %>% 
    summarise(total_qnt_tons = total_qnt_kg/1000) %>% 
    ungroup()


    datatable(producao_sec_h_provincia_dta[ order(producao_sec_h_provincia_dta$total_qnt_tons, decreasing = T), ]) %>%
        formatRound("total_qnt_tons") %>% 
        formatStyle(
            columns = "total_qnt_tons",
            background = styleColorBar(range(producao_sec_h_provincia_dta$total_qnt_tons), 'lightgreen'),
    ...
    # tabelas: Producao por distritos
    # producao_sec_h_distrito_dta <- 
    #     tipo_producao_sec_h_distrito_tbl %>% 
    #     # filter(tipo_cultura == "AMENDOIM PEQUENO") %>% 
    #     group_by(provincia=PROVINCIA, prov_dist, tipo_cultura) %>% 
    #     summarise(total_qnt_tons = total_qnt_kg/1000) %>% 
    #     ungroup()
    # 
    # tipo_producao_sec_h_distrito_merged_tbl@data %>% glimpse()
    ...
    producao_sec_h_distrito_dta <- 
    tipo_producao_sec_h_distrito_merged_tbl@data %>% 
        group_by(provincia = PROVINCIA, distrito = DISTRITO, tipo_cultura) %>% 
        summarise(total_qnt_tons = total_qnt_kg/1000) %>% 
        ungroup()
    
    datatable(producao_sec_h_distrito_dta[ order(producao_sec_h_distrito_dta$total_qnt_tons, decreasing = T), ]) %>%
        formatRound("total_qnt_tons") %>% 
        formatStyle(
            columns = "total_qnt_tons",
            background = styleColorBar(range(producao_sec_h_distrito_dta$total_qnt_tons), 'lightgreen'),
            backgroundSize = '98% 88%',
            backgroundRepeat = 'no-repeat',
            backgroundPosition = 'center'
        )    
