##################################################################################
### 07_producao_culturas_alimentares_basicas.R
##################################################################################
...
dir_name <- "C:/Users/REINALDO ZEZELA/Documents/projects/IAI2020/Reports/anuario_estatistica_agraria_2020/seccao_H/"
seccao_mapa_provincia <- "seccao_h_mapa_provincia_"
seccao_mapa_distrito <- "seccao_h_mapa_distrito_"
seccao_tabela_provincia <- "seccao_h_tabela_provincia_"
seccao_tabela_distrito <- "seccao_h_tabela_distrito_"
...
# ibsa_sec_i_dat <- read_dta("shiny_app/00_data/seccao/sec_iv1.dta")

ibsa_sec_h_dat <- read_dta("shiny_app/00_data/seccao/sec_hv1.dta")

ibsa_sec_dat <- ibsa_sec_h_dat
...
v_producao_tipo <- "OUTROS FEIJOES"
...
paletteNum <- colorNumeric('Greens', domain = tipo_producao_sec_provincia_merged_tbl$total_qnt_kg/1000)


# labels
producao_Labels <- sprintf('<b>%s</b><br/>%s ton.<br/>%s',
                           tipo_producao_sec_provincia_merged_tbl$PROVINCIA, 
                           prettyNum(round(tipo_producao_sec_provincia_merged_tbl$total_qnt_kg/1000, 0),big.mark = ","),
...
m_province <- leaflet() %>%
    ...
    addPolygons(data = tipo_producao_sec_provincia_merged_tbl,
                ...
                fillColor = ~paletteNum(tipo_producao_sec_provincia_merged_tbl$total_qnt_kg/1000),
    ...
    addLegend(pal = paletteNum, values = tipo_producao_sec_provincia_merged_tbl$total_qnt_kg/1000, title = paste0("<small>Campanha Agricola 2020 <br> Producao de ", unique(tipo_producao_sec_provincia_merged_tbl$tipo_producao), " (ton.)<br>(fonte: IAI 2020)</small>"), position = 'bottomright')

# m_province


# Save for HTML: mapa por Provincia
file_name <- paste0(dir_name, seccao_mapa_provincia, v_producao_tipo, ".html")
...
# top20 district
top10_district <-
    tipo_producao_sec_distrito_merged_tbl@data[order(tipo_producao_sec_distrito_merged_tbl$total_qnt_kg,
...
paletteNum <- colorNumeric('Greens', domain = tipo_producao_sec_distrito_merged_tbl$total_qnt_kg/1000)


producao_Labels <- sprintf('<b>%s</b><br/>%s ton.<br/>%s',
                           tipo_producao_sec_distrito_merged_tbl$DISTRITO, 
                           prettyNum(round(tipo_producao_sec_distrito_merged_tbl$total_qnt_kg/1000, 0),big.mark = ","),
...
addProviderTiles(providers$CartoDB.PositronNoLabels)  %>%
    ...
    addPolygons(data = tipo_producao_sec_distrito_merged_tbl,
                ...
                # specify that the each state should be colored per paletteNum()
                fillColor = ~paletteNum(tipo_producao_sec_distrito_merged_tbl$total_qnt_kg/1000),
    ...
    addLegend(pal = paletteNum, values = tipo_producao_sec_distrito_merged_tbl$total_qnt_kg/1000, title = paste0("<small>Campanha Agricola 2020 <br> Producao de ", unique(tipo_producao_sec_distrito_merged_tbl$tipo_producao), " (ton.)<br>(fonte: IAI 2020)</small>"), position = 'bottomright')

# m_distritos


# Save for HTML: mapa por distritos
file_name <- paste0(dir_name, seccao_mapa_distrito, v_producao_tipo, ".html")
...
producao_sec_provincia_dta <- 
    ...
    summarise(total_qnt = total_qnt_kg/1000) %>% 
    ungroup()
...
file_name <- paste0(dir_name, seccao_tabela_provincia, v_producao_tipo, ".html")
...
file_name <- paste0(dir_name, seccao_tabela_distrito, v_producao_tipo, ".html")
